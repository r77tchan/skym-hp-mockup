async (page, options = {}) => {
  const { pageId = 'it-solution', livePath = '/service/it-solution', root = '#skym-svsub', version = 'v02', outputDir = '/tmp/skym-baseline-qa' } = options;
  ...
  try {
    ...
    for (const width of [375, 400, 820, 1280, 1455]) {
      ...
      for (const [name, url] of [[version, 'http://127.0.0.1:8765/' + pageId + '/' + version + '/'], ['live', 'https://skym.co.jp' + livePath]]) {
        await p.goto(url);
        ...
        const data = await p.evaluate((root) => {
          const selectors = ['.l-header', '.l-titlebar', '.l-main', root, root + ' > section', root + ' h2', root + ' p', '.l-footer', '.l-footer a', '.l-header a'];
          ...
          return {height: document.documentElement.scrollHeight, links: [...document.querySelectorAll('a[href]')].map(a=>({text:a.textContent.trim(),href:a.getAttribute('href')})), width: innerWidth, client: document.documentElement.clientWidth, scroll: document.documentElement.scrollWidth, nodes,
    ...
    for (let i = 0; i < out.length; i += 2) {
      ...
      result.push({width: a.width, client: b.client, overflow: b.scroll > b.client, brokenImages: b.brokenImages, badStyle: b.badStyle,
        nodeCounts: [a.nodes.length,b.nodes.length], differences: a.nodes.flatMap((n,j) => JSON.stringify(n) === JSON.stringify(b.nodes[j]) ? [] : [{reference:n,live:b.nodes[j]}])});
    }
