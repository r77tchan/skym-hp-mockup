...
/top/draft/               index.html   トップページの作業中の最新版(これだけ編集する)
/top/v01/, /top/v02/ …    index.html   節目で draft を凍結したコピー(以後変更しない)
/top/source/                           参照用の原本(公開ページの無加工スナップショット、WP 本文の VC ショートコード全文)
/<page>/…                              別ページも同じ型(current / draft / vNN / source)
```
...
- `current/` と `vNN/` は**凍結**(変更しない)。編集するのは `draft/` だけ
- 節目で凍結する手順: `cp -r top/draft top/v01` → `versions.js` の該当ページに `{ id: 'v01', date: '…', note: '何を変えた版か' }` を足す → commit → push
- `draft/` を最初に作るとき: `cp -r top/current top/draft` → `versions.js` に `{ id: 'draft', date: '', note: '作業中の最新版' }` を足す
...
- 幅指定・比較: http://127.0.0.1:8765/preview.html?a=top/draft/&b=top/current/&w=390 (ヘッダーで幅・表示を切替。`b` には URL も指定可)
